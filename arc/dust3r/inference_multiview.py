...
from arc.dust3r.utils.device import collate_with_cat, to_cpu
from arc.dust3r.utils.geometry import depthmap_to_pts3d, geotrf
from arc.dust3r.utils.misc import invalid_to_nans
...
def inference(multiple_views_in_one_sample, model, device, dtype, verbose=True, profiling=False, use_center_as_anchor=False):
    ...
    if use_center_as_anchor:
        center_image_id = len(multiple_views_in_one_sample) // 2
        center_image = multiple_views_in_one_sample[center_image_id]
        multiple_views_in_one_sample.pop(center_image_id)
        multiple_views_in_one_sample.insert(0, center_image)
    ...
    # this would cause a bug as tracking rgb is still using the first view, but we just ignore it for now
    if use_center_as_anchor:
        # k is "preds", "views", "losses" (could be None)
        for k, v in result.items():
            if v is not None:
                center_result = v[0]
                result[k] = v[1:]
                result[k].insert(center_image_id, center_result)
